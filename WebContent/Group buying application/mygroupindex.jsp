					...
					<div class="ddxq">
						...
						<div class="ztxx fr">
							<ul>
								...
								<li>${order.paystate==0?"未受理":"已受理"}</li>
								<li>
									<c:if test="${order.paystate==0 }">
										<a
											href="${pageContext.request.contextPath}/delgroupOrderById?id=${order.id}"
											onclick="javascript:return o_del()">刪除</a>
									</c:if> <c:if test="${order.paystate!=0 }">
										<a
											href="${pageContext.request.contextPath}/delgroupOrderById?id=${order.id}&type=client"
											onclick="javascript:return o_del()">刪除</a>
									</c:if></li>
								<div class="clear"></div>
							</ul>
